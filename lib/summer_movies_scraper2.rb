  ...
  movie_list = doc.css("div.caption")


  ### get all the text
  def get_movie_text(movie_list)
    content = movie_list.css('div.caption').collect do |i|
        i.content
    end 
  ...
  def get_links(movie_list)
    links = movie_list.css('a').collect do |a_tag|
...
movies = get_movie_text(movie_list)
links = get_links(movie_list)
  ...
  #got dates
    ...
    # - classes needed 
        # parser class 
          ...
          #builds movie class
        # Movie class
          #stores info about itself 
        #CLI class 
          #allows you to see what movies are coming up this weekend, week, 2 weeks, and month
